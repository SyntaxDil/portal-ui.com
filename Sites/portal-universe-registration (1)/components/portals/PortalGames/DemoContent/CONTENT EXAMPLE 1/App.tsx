...
export const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-cyan-300 font-sans p-4 bg-cover bg-center" style={{backgroundImage: "url('https://images.pexels.com/photos/998641/pexels-photo-998641.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2')"}}>
      ...
      <div className="relative container mx-auto flex flex-col h-[calc(100vh-2rem)]">
        <Header souls={ship.souls} />
        <nav className="flex border-b-2 border-[var(--border-color)] mb-2">
            <button onClick={() => setActiveView('game')} className={`flex-1 text-sm font-display tracking-widest p-2 transition-colors ${activeView === 'game' ? 'text-white bg-cyan-800/40' : 'text-cyan-500 hover:bg-cyan-900/50'}`}>COMMAND</button>
            <button onClick={() => setActiveView('shipyard')} className={`flex-1 text-sm font-display tracking-widest p-2 transition-colors ${activeView === 'shipyard' ? 'text-white bg-cyan-800/40' : 'text-cyan-500 hover:bg-cyan-900/50'}`}>SHIPYARD</button>
            <button onClick={() => setActiveView('galaxy')} className={`flex-1 text-sm font-display tracking-widest p-2 transition-colors ${activeView === 'galaxy' ? 'text-white bg-cyan-800/40' : 'text-cyan-500 hover:bg-cyan-900/50'}`}>GALAXY MAP</button>
        </nav>
        <div className="flex-grow relative overflow-hidden">
            {activeView === 'game' && (
             ...
             {activeView === 'shipyard' && <div className="p-4 flex items-center justify-center h-full"><p className="text-2xl font-display animate-pulse">Full-screen shipyard coming soon...</p></div>}
             {activeView === 'galaxy' && <div className="p-4 flex items-center justify-center h-full"><p className="text-2xl font-display animate-pulse">Galaxy Map coming soon...</p></div>}
        </div>
      </div>
    </div>
  );
